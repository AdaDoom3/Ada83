...
package Text_IO is
  ...
  package Integer_IO is
    ...
    procedure Get (File : in File_Type; Item : out Number; Width : in Field := 0);
    procedure Get (From : in String; Item : out Number; Last : out Positive);
    procedure Get (Item : out Number; Width : in Field := 0);
    procedure Put (File : in File_Type; Item : in Number; Width : in Field := Default_Width; Base : in Number_Base := Default_Base);
    procedure Put (Item : in Number; Width : in Field := Default_Width; Base : in Number_Base := Default_Base);
    procedure Put (To : out String; Item : in Number; Base : in Number_Base := Default_Base);
  end;
  generic
  ...
  package Float_IO is
    ...
    procedure Get (From : in String; Item : out Number; Last : out Positive);
    procedure Get (Item : out Number; Width : in Field := 0);
    procedure Put (File : in File_Type; Item : in Number; Fore : in Field := Default_Fore; Aft : in Field := Default_Aft; Exp : in Field := Default_Exp);
    procedure Put (Item : in Number; Fore : in Field := Default_Fore; Aft : in Field := Default_Aft; Exp : in Field := Default_Exp);
    procedure Put (To : out String; Item : in Number; Aft : in Field := Default_Aft; Exp : in Field := Default_Exp);
  end;
  generic
...
package body Text_IO is
  ...
  function C_Fwrite (Pointer : System.Address; Size : Integer; Count : Integer;
  ...
  procedure Init_Standard_File (Table_Slot : Integer; Stream : System.Address;
  Mode : File_Mode) is
    begin
...
begin
  ...
  for I in 1 .. Integer (Spacing) loop
    ...
    if File_Control_Blocks (Table_Slot).Page_Length /= Unbounded and
      File_Control_Blocks (Table_Slot).Line > Integer (File_Control_Blocks (Table_Slot).Page_Length) then
      Raw_Put (Table_Slot, 12);
      File_Control_Blocks (Table_Slot).Line := 1;
...
begin
  ...
  if File_Control_Blocks (Table_Slot).Line_Length /= Unbounded and
    File_Control_Blocks (Table_Slot).Col > Integer (File_Control_Blocks (Table_Slot).Line_Length) then
    New_Line (File);
  end if;
...
begin return One_Character >= '0' and One_Character <= '9'; end Is_Dig;
...
procedure Read_Number_Token (Table_Slot : Integer; Width : Integer;
Allow_Point     : Boolean;
Scratch         : out String; Text_Length : out Integer) is
  Local                                  : String (1 .. 256);
P                                      : Integer := 0;
...
is
  ...
  Rounded_Digits : array (0 .. Max_Significant_Digits) of Integer :=
    (others => 0);
  ...
  Exact_Digits : array (0 .. Max_Exact_Digits) of Integer :=
    (others => 0);
...
begin
  if Magnitude /= 0.0 then
    ...
    begin
      while Remaining_Mantissa > 0 loop
        Exact_Digits (Exact_Digit_Count) :=
          Integer (Remaining_Mantissa mod 10);
    ...
    begin
      ...
      for Scale_Step in 1 .. Scale_Count loop
        ...
        for Digit_Index in 0 .. Exact_Digit_Count - 1 loop
          declare
            Product : constant Integer :=
              Exact_Digits (Digit_Index) * Scale_Factor + Carry;
          begin
  ...
  if Exp > 0 then
    ...
    Significant_Digit_Count :=
      Decimal_Exponent + 1 + Fraction_Digit_Count;
  else
    Significant_Digit_Count :=
      Fraction_Digit_Count + Decimal_Exponent + 1;
  end if;
  ...
  for Digit_Index in 0 .. Significant_Digit_Count - 1 loop
    if Digit_Index < Exact_Digit_Count then
      Rounded_Digits (Digit_Index) :=
        Exact_Digits (Exact_Digit_Count - 1 - Digit_Index);
    else
  ...
  if Significant_Digit_Count < Exact_Digit_Count then
    Rounding_Digit :=
      Exact_Digits
        (Exact_Digit_Count - 1 - Significant_Digit_Count);
  else
    Rounding_Digit := 0;
  end if;
  ...
  if Rounding_Digit >= 5 then
    ...
    while Carry = 1 and then Rounding_Index >= 0 loop
      Rounded_Digits (Rounding_Index) :=
        Rounded_Digits (Rounding_Index) + 1;
  ...
  for Padding_Position
    in Integer_Digit_Count + Sign_Character_Count + 1 .. Fore
  loop
  ...
  if Exp = 0 and then Decimal_Exponent < 0 then
    ...
    for Digit_Index in 0 .. Integer_Digit_Count - 1 loop
      ...
      Result_Buffer (Result_Length) :=
        Character'Val
          (Character'Pos ('0') + Rounded_Digits (Digit_Index));
    end loop;
  end if;
  ...
  begin
    ...
    for Fraction_Position in 1 .. Fraction_Digit_Count loop
      Source_Digit_Index :=
        Decimal_Point_Index + Fraction_Position;

      Result_Length := Result_Length + 1;

      if Source_Digit_Index
           in Rounded_Digits'Range
      then
        Result_Buffer (Result_Length) :=
          Character'Val
            (Character'Pos ('0')
             + Rounded_Digits (Source_Digit_Index));
      else
        Result_Buffer (Result_Length) := '0';
      end if;
    end loop;
  ...
  if Exp > 0 then
    ...
    declare
      Exponent_Digits : array (0 .. 15) of Integer :=
        (others => 0);
    ...
    begin
      if Remaining_Exponent = 0 then
        ...
        while Remaining_Exponent > 0 loop
          Exponent_Digits (Exponent_Digit_Count) :=
            Remaining_Exponent mod 10;
      ...
      for Digit_Index in reverse 0 .. Exponent_Digit_Count - 1 loop
        ...
        Result_Buffer (Result_Length) :=
          Character'Val
            (Character'Pos ('0') + Exponent_Digits (Digit_Index));
      end loop;
    end;
  end if;
...
procedure Format_Real (File : File_Type; Plain_Value : Long_Real;
  ...
  loop
    ...
    if I <= Last and then S (I) = '.' then
      ...
      package body Integer_IO is
        ...
        procedure Get (File : in File_Type; Item : out Number; Width : in Field := 0) is
          Table_Slot  : Integer := File.Handle;
        ...
        procedure Get (Item : out Number; Width : in Field := 0) is
        begin
          Get (File_Type'(Handle => Current_Input_Slot), Item, Width);
        end;
        procedure Put (File : in File_Type; Item : in Number;
      ...
      procedure Put (Item : in Number; Width : in Field := Default_Width;
      Base : in Number_Base := Default_Base) is
        begin
      Put ((Handle => Current_Output_Slot), Item, Width, Base);
    end;
    procedure Get (From : in String; Item : out Number; Last : out Positive) is
    ...
    procedure Put (To : out String; Item : in Number;
    Base : in Number_Base := Default_Base) is
...
procedure Put (Item : in Number; Fore : in Field := Default_Fore;
...
procedure Put (To : out String; Item : in Number; Aft : in Field := Default_Aft;
Exp : in Field := Default_Exp) is
...
package body Direct_IO is
  ...
  function C_Fclose (Stream : System.Address) return Integer;
  pragma Import (C, C_Fclose, "fclose");
  function C_Fread (Pointer : System.Address; Size : Integer; Count : Integer;
  Stream : System.Address) return Integer;
  ...
  procedure Seek_To (Table_Slot : Integer; Element_Index : Positive_Count) is
    Offset : Integer := Header_Bytes
      + Integer (Element_Index - 1) * (4 + File_Control_Blocks (Table_Slot).Slot);
    Ignore : Integer;
  begin
...
procedure Read_At (Table_Slot : Integer; Item : out Element_Type;
...
procedure Write_At (Table_Slot : Integer; Item : in Element_Type;
