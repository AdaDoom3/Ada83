#!/bin/bash
set -euo pipefail;declare -A X=([a]=0 [b]=0 [c]=0 [d]=0 [e]=0 [l]=0 [f]=0 [s]=0 [z]=0 [ec]=0 [ee]=0 [t]=$(date +%s%3N) [ta]=0 [tb]=0 [tc]=0 [td]=0 [te]=0 [tl]=0)
...
[bB])((++X[tb]));if timeout 0.2 ./ada83 -Iacats -Irts "$f" >acats_logs/$n.ll 2>acats_logs/$n.err;then E FAIL "$n" WRONG_ACCEPT "compiled when should reject";((++X[f]))
...
[eE])((++X[te]));timeout 0.2 ./ada83 -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err||{ E SKIP "$n" COMPILE "$(head -1 acats_logs/$n.err 2>/dev/null|cut -c1-50)";((++X[s]));return;}
timeout 0.2 llvm-link -o test_results/$n.bc test_results/$n.ll rts/report.ll 2>/dev/null||{ E SKIP "$n" BIND;((++X[s]));return;}
...
printf " %-22s %6s %6s %6s %6s %7s\n" "----------------------" "------" "------" "------" "------" "-------"
((X[ta]>0))&&printf " %-22s %6d %6s %6s %6d %6d%%\n" "A  Acceptance" ${X[a]} "-" "-" ${X[ta]} $(: ${X[a]} ${X[ta]})
((X[tb]>0))&&printf " %-22s %6d %6s %6s %6d %6d%%\n" "B  Illegality" ${X[b]} "-" "-" ${X[tb]} $(: ${X[b]} ${X[tb]})
((X[tc]>0))&&printf " %-22s %6d %6s %6s %6d %6d%%\n" "C  Executable" ${X[c]} "-" "-" ${X[tc]} $(: ${X[c]} ${X[tc]})
((X[td]>0))&&printf " %-22s %6d %6s %6s %6d %6d%%\n" "D  Numerics" ${X[d]} "-" "-" ${X[td]} $(: ${X[d]} ${X[td]})
((X[te]>0))&&printf " %-22s %6d %6s %6s %6d %6d%%\n" "E  Inspection" ${X[e]} "-" "-" ${X[te]} $(: ${X[e]} ${X[te]})
((X[tl]>0))&&printf " %-22s %6d %6s %6s %6d %6d%%\n" "L  Post-compilation" ${X[l]} "-" "-" ${X[tl]} $(: ${X[l]} ${X[tl]})
printf " %-22s %6s %6s %6s %6s %7s\n" "----------------------" "------" "------" "------" "------" "-------"
