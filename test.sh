...
[bB])if timeout 0.2 ./ada83 -Iacats -Irts "$f" >acats_logs/$n.ll 2>acats_logs/$n.err;then E FAIL "$n" WRONG_ACCEPT "compiled when should reject";((++X[f]))
else q=$(^ "$f");h=${q%:*};x=${q#*:};p=$(: $h $x);((p>=90))&&{ ((++X[b]));[[ $v == v ]]&&@ "$f"||E PASS "$n" REJECTED "$h/$x errors (${p}%)";}||{ ((++X[f]));E FAIL "$n" LOW_COVERAGE "$h/$x errors (${p}%)";};fi;;
[cC])if ! timeout 0.2 ./ada83 -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
E SKIP "$n" COMPILE "$(head -1 acats_logs/$n.err 2>/dev/null|cut -c1-50)";((++X[s]));return;fi
if ! timeout 0.2 llvm-link -o test_results/$n.bc test_results/$n.ll rts/report.ll 2>acats_logs/$n.link;then
E SKIP "$n" BIND "unresolved symbols";((++X[s]));return;fi
...
[dD])if ! timeout 0.2 ./ada83 -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
...
[eE])timeout 0.2 ./ada83 -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err||{ E SKIP "$n" COMPILE "$(head -1 acats_logs/$n.err 2>/dev/null|cut -c1-50)";((++X[s]));return;}
timeout 0.2 llvm-link -o test_results/$n.bc test_results/$n.ll rts/report.ll 2>/dev/null||{ E SKIP "$n" BIND;((++X[s]));return;}
...
[lL])if timeout 0.2 ./ada83 -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
