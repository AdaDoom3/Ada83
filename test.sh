...
while IFS=: read -r _ n _;do a+=($n);done< <(./ada83new -Iacats -Irts "$f" 2>&1|grep "^[^:]*:[0-9]")
...
while IFS=: read -r _ m _;do a+=($m);done< <(./ada83new -Iacats -Irts "$f" 2>&1|grep "^[^:]*:[0-9]");local xe=${#x[@]} ae=${#a[@]};printf "\n   %s\n" "${'':-<68>}"
...
printf "   %s\n   coverage %d/%d (%d%%)  %s\n\n" "${'':-<68>}" $h $xe $p "$v";}
R(){ [[ ! -f rts/report.ll || rts/report.adb -nt rts/report.ll ]]&&./ada83new -Iacats -Irts rts/report.adb>rts/report.ll 2>/dev/null||true;}
T(){ local f=$1 v=${2:-} n=$(basename "$f" .ada);local q=${n:0:1};if [[ $n =~ [0-9]$ && ! $n =~ m$ ]];then return;fi;((++X[z]));R;case $q in
[aA])((++X[ta]));if ! timeout 0.2 ./ada83new -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
...
[bB])((++X[tb]));if timeout 0.2 ./ada83new -Iacats -Irts "$f" >acats_logs/$n.ll 2>acats_logs/$n.err;then E FAIL "$n" WRONG_ACCEPT "compiled when should reject";((++X[f]));((++X[fb]))
else q=$(^ "$f");h=${q%:*};x=${q#*:};p=$(: $h $x);((p>=90))&&{ ((++X[b]));[[ $v == v ]]&&@ "$f"||E PASS "$n" REJECTED "$h/$x errors (${p}%)";}||{ ((++X[f]));((++X[fb]));E FAIL "$n" LOW_COVERAGE "$h/$x errors (${p}%)";};fi;;
[cC])((++X[tc]));if ! timeout 0.2 ./ada83new -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
...
[dD])((++X[td]));if ! timeout 0.2 ./ada83new -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
...
[eE])((++X[te]));timeout 0.2 ./ada83new -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err||{ E SKIP "$n" COMPILE "$(head -1 acats_logs/$n.err 2>/dev/null|cut -c1-50)";((++X[s]));((++X[se]));return;}
...
[lL])((++X[tl]));if timeout 0.2 ./ada83new -Iacats -Irts "$f">test_results/$n.ll 2>acats_logs/$n.err;then
if timeout 0.2 llvm-link -o test_results/$n.bc test_results/$n.ll rts/report.ll 2>acats_logs/$n.link;then
