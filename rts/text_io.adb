...
PRAGMA IMPORT(LLVM,RT_IMAGE_INT,"__ada_image_int");
FUNCTION RT_VALUE_INT(S:STRING)RETURN INTEGER;
PRAGMA IMPORT(LLVM,RT_VALUE_INT,"__ada_value_int");
FUNCTION RT_IMAGE_ENUM(VAL:INTEGER;WIDTH:INTEGER;UPPER:BOOLEAN)RETURN STRING;
PRAGMA IMPORT(LLVM,RT_IMAGE_ENUM,"__ada_image_enum");
...
IF LAST>0 THEN ITEM:=NUM(RT_VALUE_INT(BUF(1..LAST)));ELSE ITEM:=NUM'FIRST;END IF;
END GET;
PROCEDURE GET(ITEM:OUT NUM;WIDTH:IN FIELD:=0)IS BEGIN GET(STANDARD_INPUT,ITEM,WIDTH);END GET;
PROCEDURE PUT(FILE:IN FILE_TYPE;ITEM:IN NUM;WIDTH:IN FIELD:=DEFAULT_WIDTH;BASE:IN NUMBER_BASE:=DEFAULT_BASE)IS
S:STRING:=RT_IMAGE_INT(INTEGER(ITEM),INTEGER(WIDTH),INTEGER(BASE));
BEGIN
PUT(FILE,S);
END PUT;
...
BEGIN
ITEM:=NUM(RT_VALUE_INT(FROM));
LAST:=FROM'LAST;
END GET;
PROCEDURE PUT(TO:OUT STRING;ITEM:IN NUM;BASE:IN NUMBER_BASE:=DEFAULT_BASE)IS
S:STRING:=RT_IMAGE_INT(INTEGER(ITEM),TO'LENGTH,INTEGER(BASE));
LEN:INTEGER:=S'LENGTH;
BEGIN
IF LEN>TO'LENGTH THEN LEN:=TO'LENGTH;END IF;
FOR J IN 1..LEN LOOP
TO(TO'FIRST+J-1):=S(S'FIRST+J-1);
END LOOP;
FOR J IN LEN+1..TO'LENGTH LOOP
TO(TO'FIRST+J-1):=' ';
END LOOP;
END PUT;
END INTEGER_IO;
...
ITEM:=ENUM'FIRST;
END GET;
PROCEDURE GET(ITEM:OUT ENUM)IS BEGIN GET(STANDARD_INPUT,ITEM);END GET;
PROCEDURE PUT(FILE:IN FILE_TYPE;ITEM:IN ENUM;WIDTH:IN FIELD:=DEFAULT_WIDTH;SET:IN TYPE_SET:=DEFAULT_SETTING)IS
S:STRING:=RT_IMAGE_ENUM(ENUM'POS(ITEM),INTEGER(WIDTH),SET=UPPER_CASE);
BEGIN
PUT(FILE,S);
END PUT;
...
S:STRING:=RT_IMAGE_ENUM(ENUM'POS(ITEM),TO'LENGTH,SET=UPPER_CASE);
LEN:INTEGER:=S'LENGTH;
BEGIN
IF LEN>TO'LENGTH THEN LEN:=TO'LENGTH;END IF;
FOR J IN 1..LEN LOOP
