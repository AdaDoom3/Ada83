...
IF FROM>FILES(I).SZ OR FROM>1000 THEN RAISE END_ERROR;END IF;
ITEM:=BUFFERS(I)(POSITIVE(FROM));
FILES(I).IDX:=FROM+1;
END IF;
END READ;
...
IF FILES(I).M=OUT_FILE THEN RAISE MODE_ERROR;END IF;
IDX:=FILES(I).IDX;
IF IDX>FILES(I).SZ OR IDX>1000 THEN RAISE END_ERROR;END IF;
ITEM:=BUFFERS(I)(POSITIVE(IDX));
FILES(I).IDX:=FILES(I).IDX+1;
END IF;
END READ;
...
IF TO<=1000 THEN
BUFFERS(I)(POSITIVE(TO)):=ITEM;
IF TO>FILES(I).SZ THEN FILES(I).SZ:=TO;END IF;
FILES(I).IDX:=TO+1;
END IF;
END IF;
END WRITE;
...
IDX:=FILES(I).IDX;
IF IDX<=1000 THEN
BUFFERS(I)(POSITIVE(IDX)):=ITEM;
IF IDX>FILES(I).SZ THEN FILES(I).SZ:=IDX;END IF;
FILES(I).IDX:=FILES(I).IDX+1;
END IF;
END IF;
END WRITE;
